...
# Крошечный веб-сервер для обмана Render (чтобы работал на бесплатном тарифе)
...
async def start_web_server():
    ...
    await runner.setup()
    # Слушаем порт 8080 (или тот, что выдаст Render)
...
async def process_phone(message: Message, state: FSMContext):
    ...
    await message.answer("Выберите категорию вашей проблемы:", reply_markup=category_keyboard)
...
async def process_category(callback: CallbackQuery, state: FSMContext):
    categories = {
        "cat_phone": "📱 Ремонт смартфона", "cat_laptop": "💻 Ремонт ноутбука",
        "cat_pc": "🖥️ Ремонт ПК", "cat_other": "⚙️ Другое"
    }
...
async def process_description(message: Message, state: FSMContext):
    ...
    await message.answer("Спасибо! Ваша заявка успешно принята.", reply_markup=ReplyKeyboardRemove())
